...
def generate_simplified_structure():
    ...
    erste_ebene_keys = random.sample(list(json_data['ersteEbene'].keys()), k=21)  # 7 einzigartige First-Level-Keys
    for key in erste_ebene_keys:
        first_level_value = json_data['ersteEbene'][key]
        second_level_value = generate_value_with_unit()  # Erzeugt ein Second-Level-Element
        third_level_value = first_level_value  # Wiederholung des First-Level-Wertes
        fourth_level_value = generate_value_with_unit()  # Neue Instanz für die vierte Spalte
        simplified_structure.append({
            "first_level": first_level_value,
            "second_level": {"value": second_level_value},
            "third_level": third_level_value,
            "fourth_level": {"value": fourth_level_value}
        })
...
template = env.get_template('gen15-to-html.html')
...
output_html_path = os.path.join(output_dir, 'doc15.html')
