...
# Beispielsweise generieren Sie eine Liste von 10 einzigartigen UUIDs
...
def generate_third_fourth_level(min_count=1, max_count=2):
    # Generiert Daten für die dritte und vierte Ebene mit kontrollierter Anzahl
    data = {}
    for _ in range(random.randint(min_count, max_count)):
        key = random.choice(["I", "II"])
        data[key] = generate_value_with_unit()
    return data
...
def generate_second_level(min_count=2, max_count=4):
    ...
    for key in keys:
        beschreibung = json_data["zweiteEbene"][key]
        zweite_ebene[beschreibung] = generate_third_fourth_level()
    return zweite_ebene
...
def generate_first_level(min_count=3, max_count=5):
    ...
    for key in keys:
        beschreibung = json_data["ersteEbene"][key]
        erste_ebene[beschreibung] = generate_second_level()
    return erste_ebene

# Beispiel zur Nutzung der generierten Datenstruktur für die Tabelle im Jinja-Template
table_data = generate_first_level(min_count=3, max_count=5)
...
template = env.get_template('gen1-to-html.html')
...
output_html_path = os.path.join(output_dir, 'doc1.html')
