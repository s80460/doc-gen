...
def generate_second_level():
    alphabet = list("abcdefghijklmnopqrstuvwxyz")  # Alphabetliste
    zweite_ebene_keys = random.sample(list(json_data['zweiteEbene'].keys()), k=random.randint(2, 4))

    zweite_ebene = {}
    used_letters = set()  # Menge zur Verfolgung verwendeter Buchstaben
    for key in zweite_ebene_keys:
        for letter in alphabet:
            if letter not in used_letters:
                zweite_ebene[letter] = generate_third_fourth_level()
                used_letters.add(letter)
                break  # Schleife beenden, wenn ein Buchstabe gefunden wurde

    return zweite_ebene

def generate_first_level():
    erste_ebene_keys = random.sample(list(json_data['ersteEbene'].keys()), k=random.randint(8, 10))
...
template = env.get_template('gen18-to-html.html')
...
output_html_path = os.path.join(output_dir, 'doc18.html')
