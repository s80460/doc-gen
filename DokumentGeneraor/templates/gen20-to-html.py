...
def generate_second_table_data(max_entries=7):
    data = []
    counter = 0
...
def generate_first_level():
    erste_ebene_keys = random.sample(list(json_data['ersteEbene'].keys()), k=random.randint(5, 6))
...
template = env.get_template('gen20-to-html.html')
...
output_html_path = os.path.join(output_dir, 'doc20.html')
