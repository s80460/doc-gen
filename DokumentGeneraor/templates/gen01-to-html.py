...
def generate_third_fourth_level():
    ...
    third_level_count = random.randint(1, 2)  # Jetzt können bis zu 4 Ebenen generiert werden
    third_level = {}
...
def generate_second_level():
    zweite_ebene_keys = random.sample(list(json_data['zweiteEbene'].keys()), k=random.randint(2, 3))
    zweite_ebene = {key: generate_third_fourth_level() for key in zweite_ebene_keys}
    return zweite_ebene

def generate_first_level():
    erste_ebene_keys = random.sample(list(json_data['ersteEbene'].keys()), k=random.randint(4, 10))
...
template = env.get_template('gen01-to-html.html')
...
output_html_path = os.path.join(output_dir, 'gen01-to-html.html')
