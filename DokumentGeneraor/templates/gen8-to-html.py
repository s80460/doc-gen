...
def generate_second_level():
    zweite_ebene_keys = random.sample(list(json_data['zweiteEbene'].keys()), k=random.randint(3, 5))
    zweite_ebene = {key: generate_third_fourth_level() for key in zweite_ebene_keys}
    return zweite_ebene

def generate_first_level():
    erste_ebene_keys = random.sample(list(json_data['ersteEbene'].keys()), k=random.randint(4, 7))
...
template = env.get_template('gen8-to-html.html')
...
output_html_path = os.path.join(output_dir, 'doc8.html')
