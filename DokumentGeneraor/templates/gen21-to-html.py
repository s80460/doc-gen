...
def generate_third_fourth_level(min_level=1, max_level=4):
    roman_numerals = ["I", "II", "III", "IV"]  # Erweiterte Liste der römischen Ziffern
    third_level_count = random.randint(min_level, max_level)  # Steuerung der Anzahl der Ebenen
    third_level = {}
...
def generate_first_level():
    erste_ebene_keys = random.sample(list(json_data['ersteEbene'].keys()), k=random.randint(2, 4))
...
def generate_random_value_and_unit():
    units = ["kg", "m", "s"]  # Beispielwerte für Einheiten
    value = random.randint(1, 1500)
    unit = random.choice(units)
    return {"value": value, "unit": unit}

def generate_random_entries_for_second_table(min_entries=1, max_entries=2):
    entries = []
    num_entries = random.randint(min_entries, max_entries)  # Bestimme die Anzahl der Einträge
    for _ in range(num_entries):
        letter = generate_random_letter().upper()  # Verwende Großbuchstaben für die Ebene
        value_and_unit = generate_random_value_and_unit()
        # Erstelle einen Eintrag als String im Format "Letter: Value Unit"
        entry = f"{letter}: {value_and_unit['value']} {value_and_unit['unit']}"
        entries.append(entry)
    return entries

def generate_second_table_data():
    data = []
    for _ in range(1, 2):
        row_data = {
            "Information": fake.sentence(),
            "Data": generate_random_entries_for_second_table(1, 3),
            "Duplicate1": generate_random_entries_for_second_table(1, 3),
            "Duplicate2": generate_random_entries_for_second_table(1, 3)
        }
        data.append(row_data)
    return data
# Bis hier hin Tabelle 2

table_data = generate_first_level()
second_table_data = generate_second_table_data()
...
template = env.get_template('gen21-to-html.html')
...
output_html_path = os.path.join(output_dir, 'doc21.html')
